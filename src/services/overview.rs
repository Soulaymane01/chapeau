...
pub const UNGROUPED_LIMIT: usize = 12;
...
pub struct Section {
    ...
    /// remains available via the roots list).
    pub collapsed: bool,
}

/// The data behind the "My System" overview.
...
/// Build the overview from the database. Returns data only; presentation is
/// up to the frontend.
pub fn build(db: &Database) -> Result<Overview> {
    let conn = db.conn();
